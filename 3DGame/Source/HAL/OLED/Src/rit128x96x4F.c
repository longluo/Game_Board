/************************************************************************************
** File: - E:\ARM\lm3s8962projects\Game\3DRayCast\Display\rit128x96x4F.c
**  
** Copyright (C), Long.Luo, All Rights Reserved!
** 
** Description: 
**      rit128x96x4.c - Drivers for the RIT 128x96x4 graphical OLED display.
** 
** Version: 1.2
** Date created: 11:50:20,28/10/2012
** Author: Long.Luo
** 
** --------------------------- Revision History: --------------------------------
** 	<author>	<data>			<desc>
...
//*****************************************************************************
//! extend API useing defines
//!
//*****************************************************************************
#define UCLEVEL_MASK 0x0F
...
//*****************************************************************************
//
//! use a 6144 char array as the fame buffer to freshen the screen
//!
//!
//! \return None.
//
//*****************************************************************************
void RIT_FreshScreen()
{
	uint32_t ulHeight = 96;
	uint8_t *pTemp = &g_pucFrame[0];
...
//*****************************************************************************
//
//! using the uclevel to clear the 6144 char array fame buffer 
//!
//!
//! \return None.
//
//*****************************************************************************
void RIT_ClearBuf(uint32_t ulLevel)
{
	uint32_t ulLoop;
	
	ulLevel |= (ulLevel << 4); //then ulLevel = 0x000000XX
	ulLevel |= (ulLevel << 8); //then ulLevel = 0x0000XXXX
	ulLevel |= (ulLevel << 16); //then ulLevel = 0xXXXXXXXX	
	
	for(ulLoop = 0; ulLoop < 6144; ulLoop += 4)
	{
		*((unsigned long *)(g_pucFrame + ulLoop)) = ulLevel;
	}
}

//*****************************************************************************
//
//! draw a dot in the fame buffer 
//!
//!
//! \return None.
//
//*****************************************************************************
void RIT_DrawDotBuf(uint32_t ulX, uint32_t ulY, uint8_t ulLevel)
{
	if (ulX & 0x01)
	{
		g_pucFrame[(ulX>>1) + (ulY<<6)] = ((g_pucFrame[(ulX>>1) + (ulY<<6)] & 0xf0) | (ulLevel & 0x0f));
	}
	else
	{
		g_pucFrame[(ulX>>1) + (ulY<<6)] = ((g_pucFrame[(ulX>>1) + (ulY<<6)] & 0x0f) | ((ulLevel & 0x0f) << 4));
	}
}



void RIT_DrawLineBuf(int32_t lX1,int32_t lY1,int32_t lX2,int32_t lY2, uint8_t ucLevel)
{
	int32_t lError, lDeltaX, lDeltaY, lYStep;
    tBoolean bSteep;
    ...
    for (; lX1 <= lX2; lX1++)
    {
        ...
        if (bSteep)
        {
            ...
            if(lY1 & 1)
            {
                g_pucFrame[(lX1 * 64) + (lY1 / 2)] = ((g_pucFrame[(lX1 * 64) + (lY1 / 2)] & 0xf0) | (ucLevel & 0x0f));
            }
            else
            {
                g_pucFrame[(lX1 * 64) + (lY1 / 2)] = ((g_pucFrame[(lX1 * 64) + (lY1 / 2)] & 0x0f) | ((ucLevel & 0x0f) << 4));
            }
        }
        else
        {
            ...
            if (lX1 & 1)
            {
                g_pucFrame[(lY1 * 64) + (lX1 / 2)] = ((g_pucFrame[(lY1 * 64) + (lX1 / 2)] & 0xf0) | (ucLevel & 0x0f));
            }
            else
            {
                g_pucFrame[(lY1 * 64) + (lX1 / 2)] = ((g_pucFrame[(lY1 * 64) + (lX1 / 2)] & 0x0f) | ((ucLevel & 0x0f) << 4));
            }
        }
...
//*****************************************************************************
//
//! draw a image in the fame buffer, ulX & 0x01 must = 0. 
//!
//!
//! \return None.
//
//*****************************************************************************
void RIT_DrawTileBuf(tile_t *pstImage)
{
	uint32_t ulLevel;
	int32_t lX ;
	int32_t lY ;
	...
	for (lY = 0; lY < pstImage->ulHeight; lY ++)
    {
        lcurrentY=pstImage->lY + lY;
		//
        // Write this row of image data.
        //
		if (((lcurrentY) > 95)|| ((lcurrentY) < 0))
		{
			continue;// if the Y position is large than 95
		}
		for (lX = 0; lX < pstImage->ulWidth; lX ++)
		{
			lcurrentX=pstImage->lX + lX;
			
			if (((lcurrentX) > 127)|| ((lcurrentX) < 0))
			{
				continue;
			}
			
			ulLevel=*((pstImage->pucImage)+(lY*((pstImage->ulWidth)>>1))+(lX>>1));
			if (lX & 0x01)
			{
				ulLevel = ulLevel & 0x0F;
			}
			else
			{
				ulLevel = ulLevel>>4;
			}
			if (ulLevel == pstImage->ulAlpha)
			{
				continue;
			}
			...
			if ((lcurrentX) & 0x01)
			{
				g_pucFrame[(lcurrentX>>1) + (lcurrentY<<6)] =  ((g_pucFrame[(lcurrentX>>1) + (lcurrentY<<6)] & 0xf0)
																| (ulLevel & 0x0f));
			}
			else
			{
				g_pucFrame[(lcurrentX>>1) + (lcurrentY<<6)] = ((g_pucFrame[(lcurrentX>>1) + (lcurrentY<<6)] & 0x0f) 
																| ((ulLevel & 0xf) << 4));
			}
		}
...
//*****************************************************************************
//
//! draw a string the fame buffer, ulX & 0x01 must = 0. 
//!	not support alpha, the color of char and background all could be decided 
//!
//! \return None.
//
//*****************************************************************************
void RIT_DrawStringBuf_F (string_t *pstStr)
{
	uint32_t ulIdx1, ulIdx2;           /*x,y scan for a char                 */
    uint32_t ulCurrentIndex, ulZeroIndex;
	                                   /*current index in the buff           */
									   /*the zero index of every char in buf */	
	uint8_t ucTemp;					   /*stroe the index of the char table   */
	const uint8_t *pcStr = pstStr->pcStr;
	                                   /*current pointer to the char in str  */
	uint32_t ulNumberOfChar = 0;	   /*how many char have been draw        */
    //
    // Loop while there are more characters in the string.
    //
    while (*pcStr != 0)
    {
        ...
        // index into the character bit-map array.
        //
		if ((ulNumberOfChar*6 + pstStr->ulX)>123)
		{
			break;		 			    /*check if (128-6) have been reach   */
		}
        ...
        {
            ucTemp -= ' ';
        }							   /*generate the index in char table    */
        //
        // Build and display the character buffer.
        //
		ulZeroIndex = ((pstStr->ulX)>>1)+(pstStr->ulY<<6)+(3 * ulNumberOfChar);
        ...
        for (ulIdx1 = 0; ulIdx1 < 6; ulIdx1 += 2)
        {
            ...
            for (ulIdx2 = 0; ulIdx2 < 8; ulIdx2++)
            {
                ulCurrentIndex = ulZeroIndex +(ulIdx1>>1)+(ulIdx2<<6);
				g_pucFrame[ulCurrentIndex] = (pstStr->ucBgrLv)|(pstStr->ucBgrLv<<4);
				/*set the current point color to the background color*/

                if (g_pucFont[ucTemp][ulIdx1] & (1 << ulIdx2))
                {
                    g_pucFrame[ulCurrentIndex] = ((pstStr->ucCharLv)<<4)|(pstStr->ucBgrLv);
                }					   /*set the higher 4 bit for even column */
                if ((ulIdx1 < 4) && (g_pucFont[ucTemp][ulIdx1 + 1] & (1 << ulIdx2)))
                {
                    g_pucFrame[ulCurrentIndex] = (g_pucFrame[ulCurrentIndex]&0xf0)|(pstStr->ucCharLv);
                }					   /*set the lower 4 bit for odd column   */
            }
        }   /*end of draw one char for loop        */
		
		ulNumberOfChar++;			   /*update the number have been draw     */
    }
}
